...
def add(A, B):
    ...
    assert A.D == B.D
    return Mat(A.D,{ d: getitem(A,d) + getitem(B,d) for d in itertools.product(A.D[0],A.D[1]) } )

def scalar_mul(M, alpha):
    "Returns the product of scalar alpha with M" 
    return Mat(M.D,{ d: alpha * getitem(M,d) for d in itertools.product(M.D[0],M.D[1]) if getitem(M,d) != 0 and alpha != 0 } )

def equal(A, B):
    "Returns true iff A is equal to B"
    assert A.D == B.D
    for d in itertools.product(A.D[0],A.D[1]):
        if getitem(A,d) != getitem(B,d):
            return False
    return True

def transpose(M):
    "Returns the transpose of M"
    return Mat((M.D[1],M.D[0]),{ (j,i): getitem(M,(i,j)) for (i,j) in itertools.product(M.D[0],M.D[1]) if getitem(M,(i,j)) != 0 } )

def vector_matrix_mul(v, M):
    "Returns the product of vector v and matrix M"
    assert M.D[0] == v.D
    return Vec(M.D[1],{ c: sum([ v[r]*getitem(M, (r,c)) for r in M.D[0] ]) for c in M.D[1] } )

def matrix_vector_mul(M, v):
    "Returns the product of matrix M and vector v"
    assert M.D[1] == v.D
    return Vec(M.D[0],{ r: sum([ v[c]*getitem(M, (r,c)) for c in M.D[1] ]) for r in M.D[0] } )

def matrix_matrix_mul(A, B):
    "Returns the product of A and B"
    assert A.D[1] == B.D[0]
    return Mat((A.D[0],B.D[1]),{ (r, c): sum([ getitem(A, (r,i)) * getitem(B, (i,c)) for i in A.D[1] ]) for (r,c) in itertools.product(A.D[0],B.D[1]) } )
