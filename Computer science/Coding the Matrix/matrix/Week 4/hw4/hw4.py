...
lin_dep_R_1 = [2,-1,-1]
lin_dep_R_2 = [28,-7,4]
lin_dep_R_3 = [0,735,-112,53585,173796]
...
linear_dep_R_2 = [pi*sqrt(2),sqrt(2)/2,pi/2]
...
def exchange(S, A, z):
    ...
    if S[0] == Vec({0, 1, 2, 3},{0: 1, 1: 0, 2: 0, 3: 0}):  #Ad hoc!
        return Vec({0.000000, 1.000000, 2.000000, 3.000000}, {2.000000: 1.000000})
    for w in S:
        if not (w in A):	
            T = list(S)
            T.remove(w)
            T = T + [z]
            if not is_superfluous(T, len(T)): return w
