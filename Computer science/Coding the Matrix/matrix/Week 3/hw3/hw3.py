...
def lin_comb_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    return Vec( M.D[0], sum([ v[i] * mat2coldict(M)[i]  for i in M.D[1] ]).f )


## Problem 12
def lin_comb_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    return Vec( M.D[1], sum([ v[i] * mat2rowdict(M)[i]  for i in M.D[0] ]).f )

## Problem 13
def dot_product_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    return Vec( M.D[0], { i: mat2rowdict(M)[i] * v for i in M.D[0] } )
...
def dot_product_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    return Vec( M.D[1], { i: v * mat2coldict(M)[i] for i in M.D[1] } )

v1 = Vec({1, 2, 3}, {1: 1, 2: 8})
M1 = Mat(({1, 2, 3}, {1, 2, 3}), {(1, 2): 2, (2, 1):-1, (3, 1): 1, (3, 3): 7})
ans1 = (dot_product_vec_mat_mult(v1, M1))
print (ans1 == Vec({1, 2, 3},{1: -8, 2: 2, 3: 0}))
v2 = Vec({'a','b'}, {})
M2 = Mat(({'a','b'}, {0, 2, 4, 6, 7}), {})
ans2 = (dot_product_vec_mat_mult(v2, M2))
print(ans2.f)

## Problem 15
def Mv_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    return coldict2mat( { c: A * mat2coldict(B)[c] for c in B.D[1] } )


## Problem 16
def vM_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    return rowdict2mat( { r: mat2rowdict(A)[r] * B for r in A.D[0] } )
