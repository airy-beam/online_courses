...
def GF2_span(D, L):
    ...
    pass
    vec_len = len(L)
    coeffs = list(product([0,one], repeat = vec_len))
    r = ([ sum([scalar_mul(L[i],c[i]) for i in range(len(c))]) for c in coeffs ])
    return ([ Vec(D,{ key: val for key, val in v.f.items() if val != 0 }) for v in r ])
